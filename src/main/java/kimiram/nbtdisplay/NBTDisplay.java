	...
	public void onInitializeClient() {
		ItemTooltipCallback.EVENT.register((stack, context, type, texts) -> {
            if (InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), KeyBindingHelper.getBoundKeyOf(showNbtKeybind).getCode())) {
                DataResult<NbtElement> result = ComponentChanges.CODEC.encodeStart(Objects.requireNonNull(context.getRegistryLookup()).getOps(NbtOps.INSTANCE), stack.getComponentChanges());
                NbtCompound compound = (NbtCompound) result.getOrThrow();
                if (!compound.isEmpty()) {
                    String raw = compound.toString();
                    String line = "";
                    int spaces = 0;
